...
def evaluate(config_name: str, layout: str, workers: int, cows: int, sheep: int):
    results = [run_episode(config_name, layout, workers, cows, sheep, s) for s in SEEDS]
    ...
    return {
        ...
        "cows_bought": statistics.mean(r["cows_bought"] for r in results),
        "sheep_bought": statistics.mean(r["sheep_bought"] for r in results),
        "acts": {
...
def main():
    configs = [
        ("CONTROL (1W, No Live)", "CONTROL", 1, 0, 0),
        ("A (1W, 1C+1S, TopLeft)", "A", 1, 1, 1),
        ("B (1W, 1C+1S, ShedAdj)", "B", 1, 1, 1),
        ("C (1W, 1C+1S, Compact)", "C", 1, 1, 1)
    ]

    all_results = []
    print("Running K3 Experiment...")
    ...
    report = "# Experiment K3 — Spatial Livestock Workload Report\n\n"
    report += "| Config | Mean Reward | Util% | Prod | Move | Melons | Milk | Wool | Fert | Cows Bought | Sheep Bought | Feed Buy |\n"
    report += "|--------|-------------|-------|------|------|--------|------|------|------|-------------|--------------|----------|\n"

    for r in all_results:
        report += f"| {r['config']} | ${r['mean_reward']:.0f} | {r['utilization']*100:.1f}% | {r['productive']:.1f} | {r['movement']:.1f} | {r['sold_melons']:.1f} | {r['sold_milk']:.1f} | {r['sold_wool']:.1f} | {r['sold_fert']:.1f} | {r['cows_bought']:.1f} | {r['sheep_bought']:.1f} | {r['feed_bought']:.1f} |\n"
    ...
    with open("k3_report.md", "w") as f:
        f.write(report)

    print("\nResults saved to k3_report.md")
