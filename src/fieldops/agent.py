...
class AgentCoordinator:
    ...
    def __call__(self, obs: dict[str, Any]) -> dict[str, Any]:
        ...
        # Experiment A-v3: Targeted Front-Running
        # We know melons mature on Day 13 (approx step 306).
        # We hire exactly 1 worker on Day 13 to harvest quickly and crash the market
        # before the Champion arrives on Day 14.
        current_day = obs.get("step", 0) // 24 + 1
        current_workers = len(state.my_farm.hands)
        
        if current_day == 1 and current_workers < 1:
            actions["market"].append(["HIRE"])
        elif current_day == 13 and current_workers < 3:
            for _ in range(3 - current_workers):
                actions["market"].append(["HIRE"])
